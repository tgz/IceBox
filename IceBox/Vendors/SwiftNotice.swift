...
extension UIViewController {
    ...
    func noticeError(text: String, autoClear: Bool = true, autoClearTime: Int = 2) {
        SwiftNotice.showNoticeWithText(NoticeType.error, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    func noticeInfo(text: String, autoClear: Bool = true, autoClearTime: Int = 10) {
...
class SwiftNotice: NSObject {
    ...
    static func showNoticeWithText(type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        let width = max(90, label.frame.width + 20)
        let frame = CGRect(x: 0, y: 0, width: width, height: 75 + label.frame.height)

        label.frame = CGRect(x: 0, y: 60, width: frame.width, height: labelFrame.height)
        ...
        checkmarkView.frame = CGRectMake(frame.width / 2 - 18, 15, 36, 36)
